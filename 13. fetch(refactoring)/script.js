...
// Fetch Refactoring (async dan await)
const buttonSearch = document.querySelector('.button-search');
buttonSearch.addEventListener('click', async function () {
  const inputKeyword = document.querySelector('.input-keyword');
  const movies = await showMovies(inputKeyword.value);
  // console.log(movies);
  updateUI(movies);
});
...
function getMovieDetail(imdbid) {
  return fetch('http://www.omdbapi.com/?apikey=6b5c8078&i=' + imdbid)
    .then(response => response.json())
    .then(m => m);
}
...
function showMovies(keyword) {
  return fetch('http://www.omdbapi.com/?apikey=6b5c8078&s=' + keyword)
    .then(response => response.json())
